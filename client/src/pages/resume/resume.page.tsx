    ...
    return (<>
        ...
        <div className="relative bg-[#F7FBFE] min-h-screen">
            ...
            <div className="relative container mx-auto print:hidden animate-pulse blur-2xl">
                ...
                <div className="absolute z-10 top-4 left-20 w-32 h-32 bg-[#f7e4b0] rounded-full"></div>
                <div className="absolute z-10 top-8 left-4 w-32 h-32 bg-[#f3acb2] rounded-full"></div>
            </div>

            {/* header with name and contact */}
            <div className="relative z-20 py-10 px-8 container mx-auto print:p-0 print:pb-8">
                {/* header content */}
                <header className="flex flex-col justify-start items-start sm:flex-row sm:justify-between sm:items-center gap-5 print:flex-row print:justify-between print:items-center">
                    {/* name */}
                    <div>
                        <h1 className="whitespace-nowrap text-2xl font-bold text-black-darker">
                            Bruno Esparza
                        </h1>
                        <h2 className="whitespace-nowrap text-lg font-medium text-black-lighter">
                            Frontend Developer
                        </h2>
                    </div>
                    {/* contact links */}
            ...
            {/* content */}
            <div className="relative z-20 p-8 container mx-auto print:p-0 print:py-8">
...
/// inline link with an icon for contact
const ContactLink: React.FC<{ link: SocialLink }> = ({ link }) => (
    <div className='flex flex-row-reverse sm:flex-row print:flex-row gap-2 '>
        {/* url text with link */}
        <a target="_blank" rel="noopener noreferrer" className="text-black-lighter underline text-sm font-medium " href={link.redirect}>
            {link.textUrl}
        </a>

        {/* icon */}
        <link.icon className="h-4 text-black-lighter" title={link.title} ></link.icon>
    </div>
);

/// row to group a characteristic
const ResumeRow: React.FC = ({ children }) => (
    <section className="relative flex flex-col gap-2 sm:flex-row sm:gap-4 print:gap-4 print:flex-row">
        {children}
    </section>
);
...
const ResumeSectionTile: React.FC = ({ children }) => (
    <h2 className="font-bold text-base text-black-darker -ml-2 sm:w-1/5 sm:ml-0 print:w-1/5 print:ml-0">
        {children}
    </h2>
);

/// section content
/// on larger screen than small, make this container take [4/5] or the remaining style of the title section
...
const ResumeSectionContentDate: React.FC = ({ children }) => (
    <div className="font-medium text-sm text-black-lighter">
        {children}
    </div>
);
